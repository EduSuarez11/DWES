...
//    }
    ...
    @GetMapping("/movies/{id}/with-cast")
    public ResponseEntity<MovieWithCastDto> getMovieByIdWithCast(@PathVariable("id") Long id) {
        return ResponseEntity.status(HttpStatus.OK).body(movieService.getMovieByIdWithCast(id));
    }
...
//    así que necesitarías otro handler en tu GlobalExceptionHandler
    ...
    public ResponseEntity<Page<MovieResponseDto>> getMovies(
            @RequestParam(required = false)
//            @Pattern(regexp = "^\\b(ACTION|DRAMA|WAR|SCI_FI)\\b$", message = "El género debe tener un formato correcto")
            Genre genre,
            @PageableDefault(size = 5, sort = "title", direction = Sort.Direction.ASC) Pageable pageable
            ){

        return ResponseEntity.status(HttpStatus.OK).body(movieService.getMovieWithCastByGenre(genre, pageable));
    }
