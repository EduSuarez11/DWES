...
//    }
    ...
    /**
     * Endpoint para obtener los actores de una película: 200
     * @param movieId
     * @return
     */
    @GetMapping("/{id}/cast")
    public ResponseEntity<List<ActorResponseDto>> listActors(
            @PathVariable("id") Long movieId
    ) {
        return ResponseEntity.status(HttpStatus.OK).body(movieCastService.listActors(movieId));
    }


    /**
     * Endpoint para actualizar la tabla intermedia: 200
     * @param movieId
     * @param actorId
     * @return
     */
    @PatchMapping("/{mId}/cast/{cId}")
    public ResponseEntity<MovieCastUpdateResponseDto> updateMovieCast(
            @PathVariable("mId") Long movieId,
            @PathVariable("cId") Long actorId,
            @RequestBody MovieCastUpdateRequestDto req
    ) {
        return ResponseEntity.status(HttpStatus.OK).body(movieCastService.updateMovieCast(movieId, actorId, req));
    }

    /**
     * Endpoint que elimina una película: 204
     * @param id
     * @return
     */
    @DeleteMapping("/{id}")
    ...
    /**
     *
     * Endpoint que lista las peliculas y sus actores: 200
     * @return
     */
    @GetMapping("/with-cast")
    public ResponseEntity<List<MovieWithCastDto>> listMoviesWithCast() {
