...
import es.daw.extra_api_peliculas.dto.request.MovieCastUpdateRequestDto;
...
import es.daw.extra_api_peliculas.dto.response.MovieCastUpdateResponseDto;
...
//            throw new ConflictException("El actor '%s' ya tiene un rol en la película '%s'".formatted(actor.getStageName(), movie.getTitle()));
//        }

//        if (alreadyExists) {
//            throw new ResponseStatusException(HttpStatus.CONFLICT, "El actor ya tiene un rol en esta película");
//        }
        if(alreadyExists){
            throw new ConflictException("El actor ya tiene un rol en esta película");
        }
    ...
    public MovieCastResponseDto addActorToMovieAndReturnsCastDto(Long movieId, MovieCastRequestDto dto) {
        ...
        Movie movie = movieRepository.findById(movieId)
                .orElseThrow(() -> new ResourceNotFoundException("Película no encontrado con id: "+movieId));

        Actor actor = actorRepository.findById(dto.actorId())
                .orElseThrow(() -> new ResourceNotFoundException("Actor no encontrado con id: " + dto.actorId()));
    ...
    /// Listar actores de una pelicula
    public List<ActorResponseDto> listActors(Long movieId) {
        movieRepository.findById(movieId)
                .orElseThrow(() -> new ResourceNotFoundException("Movie not found with id: " + movieId));

        List<ActorResponseDto> actors = movieCastRepository.getActorsByMovieId(movieId);

        return actors;
    }

    /// Listar peliculas por su actor
    public List<MovieCastResponseDto> listMovies(Long actorId) {
        actorRepository.findById(actorId)
                .orElseThrow(() -> new ResourceNotFoundException("Actor not found with id: " + actorId));

        List<MovieCastResponseDto> actors = movieCastRepository.getMoviesByActorId(actorId);
    ...
    /// Actualizar los datos de MovieCast
    @Transactional
    public MovieCastUpdateResponseDto updateMovieCast(Long movieId, Long actorId, MovieCastUpdateRequestDto dto) {
        actorRepository.findById(actorId)
                .orElseThrow(() -> new ResourceNotFoundException("Actor not found with id: " + actorId));
        ...
        MovieCast movieCast = movieCastRepository.getMovieCastByMovieIdAndActorId(movieId, actorId);

        movieCast.setScreenMinutes(dto.screenMinutes());
        movieCast.setSalaryOverride(dto.salaryOverride());

        return new MovieCastUpdateResponseDto(
                movieCast.getMovie().getId(),
                movieCast.getActor().getId(),
                movieCast.getCharacterName(),
                movieCast.getScreenMinutes(),
                movieCast.getSalaryOverride()
        );
    }
